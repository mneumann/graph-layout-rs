...
pub fn layout<F,V>(step_fn: F, max_iter: usize, converge_eps: f32, k_r: f32, k_s: f32, min_pos: &V, max_pos: &V,
...
pub fn layout_typical_2d(l: Option<f32>, node_positions: &mut[P2d], node_neighbors: &[Vec<usize>]) {
    let n = node_positions.len();
    assert!(node_neighbors.len() == n);
    ...
    layout(step_fn, MAX_ITER, EPS, k_r, k_s, &min_pos, &max_pos, node_positions, node_neighbors);
}
